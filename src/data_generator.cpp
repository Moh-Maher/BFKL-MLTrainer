...
int main(void)
{
	...
	if (choice == 1) {
		Output mydataLLABFKL(path_to_LLA_data);
		mydataLLABFKL.takeData<<"n,nu,Y,MuR,LLA_Kernel"<<std::endl;
		...
		for (int Y_int = static_cast<int>(Y_start * 10); Y_int <= static_cast<int>(Y_end * 10); Y_int += static_cast<int>(Y_step * 10)) {
			...
			for (int j = j_start; j <= j_end; ++j) {
			    mydataLLABFKL.takeData << n << "," << j << "," << Y << "," << MUR << "," << std::setprecision(16) << std::real(BFKL.LLA_Kernel(n, j, Y, MUR)) << std::endl;
			}
		    }
		}
		}
		...
		Output mydataNLABFKL(path_to_NLA_data);
		mydataNLABFKL.takeData<<"n,nu,Y,MuR,NLA_Kernel"<<std::endl;
		//#pragma omp parallel for collapse(2)
		for (int n = 0; n <= 2; ++n) {
		for (int Y_int = static_cast<int>(Y_start * 10); Y_int <= static_cast<int>(Y_end * 10); Y_int += static_cast<int>(Y_step * 10)) {
		    double Y = static_cast<double>(Y_int) / 10.0; // Convert back to original floating-point value
		  //  #pragma omp parallel for
			...
			for (int j = j_start; j <= j_end; ++j) {    
			   	if(j==0){continue;}               
			   		...
			   		mydataNLABFKL.takeData << n << "," << j << "," << Y << "," << MUR << "," << std::setprecision(16) << std::real(BFKL.NLA_Kernel(n, j, Y ,MUR, MUR, MUR*MUR))<<std::endl;
			   	}
			}
		    }
		}
